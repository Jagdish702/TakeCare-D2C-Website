...
/*
  ...
  14012:23587, reached from either option on GiftPage ("Yes, It's a gift"
  or "No, it's not" both lead here — same as CaregiverConfirmPage→GiftPage).
  Reuses the shared CheckoutStepper (currentStep=1), the Delivery Van/
  Location/Phone-call icons exported from OrderDetailsPage, and the same
  stepper/progress-bar/dot-grid-card shell as the rest of this sub-flow.

  Per the explicit content mapping: the gift card's "A gift from" name and
  the Caregiver card both come from the account holder's own details
...
export default function GiftSummaryPage({ shippingInfo, personDetails, isCaregiver, isOpen, onDone }) {
  ...
  // "Caregiver" card entirely and shows only the recipient.
